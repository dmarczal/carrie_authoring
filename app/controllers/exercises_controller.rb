...
class ExercisesController < ApplicationController
  ...
  load_and_authorize_resource :find_by => :slug, :parent => true
  ...
  def destroy
    ...
    redirect_to @learning_object, notice: "Exercício deletado com sucesso"
  end
  ...
  def create_fractal(id)
    ...
    if fractal
      ...
      @exercise.build_fractal_exercise
    end
  end
  ...
  def load_params
    ...
    rules = params[:exercise][:fractal_exercise][:rules]
    unless rules.empty?
      @exercise.fractal.rules= rules.gsub(/\s/, '').split(',')
    end
  end
end
